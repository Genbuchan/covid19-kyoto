      ...
      >
        <template v-slot:infoPanel>
          <data-view-basic-info-panel
            :r-text="$t(`${confirmedCasesDate.getMonth() + 1}月${confirmedCasesDate.getDate()}日の累計`)"
          />
        </template>
...
export default {
  ...
  data() {
    // 最新の検査日
    const confirmedCasesDate = new Date(inspectionsSummary.data[inspectionsSummary.data.length - 1]['日付'])
